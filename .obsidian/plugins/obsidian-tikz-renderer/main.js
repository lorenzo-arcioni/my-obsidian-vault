...
var require_temp = __commonJS({
  "node_modules/temp/lib/temp.js"(exports, module2) {
    ...
    function mkdir2(affixes, callback) {
      var p = promisify(callback);
      var promise = p[0];
      callback = p[1];
      var dirPath = generateName(affixes, "d-");
      fs2.mkdir(dirPath, parseInt("0700", 8), function(err) {
        if (!err) {
    ...
    function mkdirSync(affixes) {
      var dirPath = generateName(affixes, "d-");
      fs2.mkdirSync(dirPath, parseInt("0700", 8));
      deleteDirOnExit(dirPath);
      return dirPath;
    }
    function open(affixes, callback) {
      ...
      fs2.open(filePath, RDWR_EXCL, parseInt("0600", 8), function(err, fd) {
        if (!err) {
          deleteFileOnExit(filePath);
        }
        callback(err, { path: filePath, fd });
      });
      return promise;
    }
    function openSync(affixes) {
      var filePath = generateName(affixes, "f-");
      var fd = fs2.openSync(filePath, RDWR_EXCL, parseInt("0600", 8));
      deleteFileOnExit(filePath);
      return { path: filePath, fd };
    }
    function createWriteStream(affixes) {
      var filePath = generateName(affixes, "s-");
      var stream = fs2.createWriteStream(filePath, { flags: RDWR_EXCL, mode: parseInt("0600", 8) });
      deleteFileOnExit(filePath);
      return stream;
    }
    exports.dir = dir;
    ...
    return __async(this, null, function* () {
      ...
      this.registerMarkdownPostProcessor((el, ctx) => {
        ...
        for (const block of Array.from(codeblocks)) {
          ...
          if (!first_line.contains("tikz render"))
            continue;
          const payload = lines.slice(1).join("\n");
          let renderMode = this.settings.defaultRenderMode;
          if (first_line.contains("side-by-side"))
            renderMode = "side_by_side";
          if (first_line.contains("image-only"))
          ...
          this.renderTikz2SVG(payload).then((data) => {
            d.innerHTML = data;
          }).catch((err) => {
...
\\end{document}`;
  ...
  renderLatex2SVG(source) {
    return new Promise((resolve, reject) => {
      temp.mkdir("obsidian-tikz", (err, dirPath) => {
        const inputPath = path.join(dirPath, "input.tex");
        fs.writeFile(inputPath, source, (err2) => {
          if (err2)
            reject(err2);
          const command = this.settings.latexCommand.replace("{input}", "input.tex").replace("{output}", "output.svg");
          (0, import_child_process.exec)(command, { cwd: dirPath }, (err3) => {
            if (err3)
              reject(err3);
            fs.readFile(path.join(dirPath, "output.svg"), function(err4, data) {
              if (err4)
                reject(err4);
...
var SettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("latex command").setDesc("Command executed to render latex to svg").addTextArea((text) => text.setValue(this.plugin.settings.latexCommand).onChange((value) => __async(this, null, function* () {
      this.plugin.settings.latexCommand = value;
      yield this.plugin.saveSettings();
    })));
    new import_obsidian.Setting(containerEl).setName("Default Render Mode").setDesc("Render Mode. You might have to refresh the rendered output for this to have an effect.").addDropdown((dropdown) => dropdown.addOption("side_by_side", "side by side").addOption("image_only", "image only").setValue(this.plugin.settings.defaultRenderMode).onChange((value) => __async(this, null, function* () {
